...
data_RF_outcome_single_tree <- function(data){
  ...
  temp <- data%>%
    ...
    mutate(age = case_when(age >= 16 & age <= 25 ~ "16_25",
                           ...
                           age > 75 & age <= 85 ~ "76_85",
                           age > 85 ~ "85_plus"))%>%
    select(age, genre, atcd.enfance, atcd.famille, std_eeg, les.epi, status.neuro, aura, veille.sommeil, res.dx.final)
  ...
  temp <- temp%>%
    dplyr::mutate(std_eeg = ifelse(std_eeg == "focale" | std_eeg == "diffuse", "Spikes", "No spikes"),
                  ...
                  atcd.enfance = ifelse(atcd.enfance == "aucun" | atcd.enfance == "aucun/autre", "None", "Pertinent history"),
...
data_RF_outcome_single_tree_LT <- function(data){
  ...
  temp <- data%>%
    ...
    mutate(age = case_when(age >= 16 & age <= 25 ~ "16_25",
                           ...
                           age > 75 & age <= 85 ~ "76_85",
                           age > 85 ~ "85_plus"))%>%
    select(age, genre, atcd.enfance, atcd.famille, std_eeg, lt_eeg, les.epi, status.neuro, aura, veille.sommeil, res.dx.final)
...
setwd("~/Analyses HUG sept 19/First_seizure/First_seizure/Article V3")
...
N_EEG <- nrow(data_full_cleaned%>%filter(!is.na(date.1st.EEG)))

pct_N_EEG_24h <- pct.fun(N_EEG_24h, N_EEG)

# N CT
N_CT <- nrow(data_full_cleaned%>%
  filter(!is.na(date.CT)))
...
delay_LT_EEG <- data_full_cleaned%>%
  mutate(event_intervention = case_when(SBB.1 == 1 ~ date.1st.1st.sz,
                                        SBB.1 == 2 ~ date.2nd.1st.sz,
                                        SBB.1 == 3 ~ date.dern.1st.sz),
         delai.lt.eeg = date.lt.EEG - event_intervention)%>%
  filter(delai.lt.eeg > 0, delai.lt.eeg < 100)%>%
  summarise(mean_days = round(mean(delai.lt.eeg, na.rm = T),2),
            sd_days = round(sd(delai.lt.eeg, na.rm = T),2),
            median_days = median(delai.lt.eeg, na.rm = T),
            max_days = max(delai.lt.eeg, na.rm = T))

# Delay MRI


delay_MRI <- data_full_cleaned%>%
  mutate(event_intervention = case_when(SBB.1 == 1 ~ date.1st.1st.sz,
                                        SBB.1 == 2 ~ date.2nd.1st.sz,
                                        SBB.1 == 3 ~ date.dern.1st.sz),
         delai.1st.irm = date.IRM - event_intervention)%>%
  filter(delai.1st.irm > 0, delai.1st.irm < 100)%>%
  summarise(mean_days = round(mean(delai.1st.irm, na.rm = T),2),
            sd_days = round(sd(delai.1st.irm, na.rm = T),2),
            median_days = median(delai.1st.irm, na.rm = T),
            max_days = max(delai.1st.irm, na.rm = T))
```


For all patients, routine EEG within 24 h was organized, and obtained in `r pct_N_EEG_24h` (N = `r N_EEG_24h`/`r N_EEG`) of the cases. CT was carried out in `r pct_CT` (N = `r N_CT`/`r tot_pat`) of the cases. As an effect of our preliminary analysis of the yield of long-term EEG (LT-EEG) and MRI from the study of Fisch et al (JOON), both exams were often already organized by the colleague in the ED and obtained on average within `r as.numeric(delay_LT_EEG$mean_days)`  (SD = `r as.numeric(delay_LT_EEG$sd_days)`) and `r as.numeric(delay_MRI$mean_days)` days (SD = `r delay_MRI$sd_days`), respectively.

```{r cleaning_methods, include=FALSE}

rm(delay_LT_EEG, delay_MRI, N_CT, N_EEG, N_EEG_24h, N_not_first_event, N_pat_adm_ED, pct_CT, pct_N_EEG_24h, pct_not_first_event, pct_pat_adm_ED)
...
```{r diagnostic tree}
##------------------------------------------------
##  Creation of the summary tables for each step  
##------------------------------------------------
...
datachi <- data_full_cleaned%>%
  filter(!is.na(veille.sommeil),
         veille.sommeil != "non documente")%>%
  mutate(dx_cat = ifelse(dx.event == "Epilepsy" & !is.na(dx.event), "epi", "non epi"))%>%
  group_by(dx_cat, veille.sommeil)%>%
  summarise(N = n())%>%
  spread(key = veille.sommeil, value = N)

datachi <- as.data.frame(datachi)
row.names(datachi) <- datachi$dx_cat
datachi <- datachi[,2:3]

chi_sleep_wake <- chisq.test(datachi)
...
ADD THE NUMBERS FOR EPILEPTIC PATIENTS

Respectively Among all patients and epileptic patients, `r N_normal` and `N_normal_epi`, had normal MRI or showed benign abnormalities (few small white matter lesions, developmental venous abnormalities, etc). `r N_les$N[N_les$les.epi == "douteux"]` of the `r sum(N_les$N)` patients with an MRI (`r pct.fun(N_les$N[N_les$les.epi == "douteux"], sum(N_les$N))`) belonged to the intermediate category. Regarding age, patients with an epileptogenic lesion and intermediate lesion were significantly older than the patients without lesion (epileptogenic lesion: `r post_hoc_les$report[9]`; intermediate: `r post_hoc_les$report[7]`. Other comparisons did not reach significance.  

Among all patients, `r N_normal` had a normal MRI or showed benign abnormalities (few small white matter lesions, developmental venous abnormalities, etc), while `` epileptic patients showed normal or non-epileptogenic lesions.`r N_les$N[N_les$les.epi == "douteux"]` of the `r sum(N_les$N)` patients with an MRI (`r pct.fun(N_les$N[N_les$les.epi == "douteux"], sum(N_les$N))`) belonged to the intermediate category. Among the epileptic patients, the proportion is more important 
...
N_tot_EEG_time <- sum(!is.na(data_full_cleaned$date.1st.EEG))

# Chi squares efficacy EEG delay all patients 
data_chi <- data_full_cleaned%>%
  filter(SBB.1 == 1)%>%
  filter(!is.na(code.pat))%>%
  mutate(EEG_delay = date.1st.EEG - date.1st.1st.sz,
         EEG_delay_cat = case_when(EEG_delay < 2 ~ "24",
                                   EEG_delay >1 & EEG_delay < 3 ~ "48",
                                   EEG_delay > 2 & EEG_delay < 4 ~ "72",
                                   EEG_delay >= 4 ~ ">72"))%>%
  mutate(EEG.result_res_ok = ifelse(EEG.result_resume == "pointes" | EEG.result_resume == "pointes+slow", "pointes", NA),
         EEG.result_res_ok = ifelse(res.1st.EEG == "normal", "normal", EEG.result_res_ok))%>%
  mutate(EEG.result_res_ok_b = ifelse(EEG.result_resume == "slow","normal", NA),
         EEG.result_res_ok = ifelse(is.na(EEG.result_res_ok), EEG.result_res_ok_b, EEG.result_res_ok))%>%
  select(-EEG.result_res_ok_b)%>%
  group_by(EEG_delay_cat, EEG.result_res_ok)%>%
  summarise(N = n())%>%
  filter(!is.na(EEG_delay_cat),
         !is.na(EEG.result_res_ok),
         !is.na(N))%>%
  spread(key = EEG.result_res_ok, value = N)
...
N_spikes_not_epi <- nrow(data_full_cleaned%>%
  filter(EEG.result_resume == "pointes" | 
           EEG.result_resume == "pointes+slow" | 
           EEG.lt.result_resume == "pointes" | 
           EEG.lt.result_resume == "pointes+slow")%>%
...
Epileptiform abnormalities were found in `r N_spikes_not_epi` patients out of `r tot_non_epi` without a diagnosis of epilepsy on the standard EEG, corresponding to `r pct.fun(N_spikes_not_epi, tot_non_epi)` of false positive results. For 3 of them, the LT EEG was normal and the epileptogenic abnormality (sharper slow waves in all cases) was not confirmed. Regarding the 4 other cases, LT EEG was either refused or not proposed to the patient, but the observed epileptiform discharges location was not concordant with the semiology of the event, leading to the exclusion of the diagnosis of epilepsy despite the observed electrical abnormalities.
...
Among the entire sample, `r data_dead_HF$N[data_dead$alive == "dead"]` patients deceased during the two year follow-up (`r pct.fun(sum(data_dead_HF$N[data_dead$alive == "dead"]), tot_pat)`; mean age = `r data_dead$mean_age[data_dead$alive =="dead"]`, `r data_dead_HF$N[data_dead_HF$alive =="dead" & data_dead_HF$genre == "F"]` females and `r data_dead_HF$N[data_dead_HF$alive =="dead" & data_dead_HF$genre == "H"]`). In the epileptic group (N = `r datachi$dead[1]`/`r tot_epi_disease`; `r pct.fun(datachi$dead[1],tot_epi_disease) `), this proportion is significantly larger than in the non-epileptic group (N = `r datachi$dead[2]`/`r tot_non_epi`; `r pct.fun(datachi$dead[2],tot_non_epi)`; `r report_chi(chi_dead)`), which is mainly due to patients with epilepsy and dementia or brain tumors. Two epileptic patients died from other causes (one of them from cardiac origin and the other one from severe hypernatremia and infection) and for six patients the cause of the death remained unclear. None of the patients experienced death related directly to an epileptic seizure. 
...
The average relapse rate (RR), i.e. the mean number of relapses among all patients is `r mean_RR` (SD = `r sd_RR`). Among epileptic patients, the rate was in average `r mean_RR_epi` (SD = `r sd_RR_epi`), while non-epiltptic patients presented a rate of `r mean_RR_non_epi` (SD = `r sd_RR_non_epi`). More precisely among diagnoses, the RR for patients with IGE was on average `r RR_per_dx$mean_RR[3]` (SD = `r RR_per_dx$sd_RR[3]`), and was significantly lower that that of NLE (RR = `r RR_per_dx$mean_RR[6]`; SD =  `r RR_per_dx$sd_RR[6]`; `r report_post_hoc_RR$report[24]`), who showed the highest RR, but significantly higher than the vagal (RR = `r RR_per_dx$mean_RR[9]`; SD = `r RR_per_dx$sd_RR[9]`; `r report_post_hoc_RR$report[27]`) and cardiovascular groups (RR = `r RR_per_dx$mean_RR[11]`; SD = `r RR_per_dx$sd_RR[11]`; `r report_post_hoc_RR$report[29]`). The SE group (RR = `r RR_per_dx$mean_RR[5]`; SD = `r RR_per_dx$sd_RR[11]`) showed higher RR than several non-epilepsy patients, namely the vasovagal syncopes (RR = `r RR_per_dx$mean_RR[9]`; SD = `r RR_per_dx$sd_RR[9]`; `r report_post_hoc_RR$report[42]`), cardiovascular (RR = `r RR_per_dx$mean_RR[11]`; SD = `r RR_per_dx$sd_RR[11]`; `r report_post_hoc_RR$report[44]`), and PNES groups (RR = `r RR_per_dx$mean_RR[12]`; SD = `r RR_per_dx$sd_RR[12]`; `r report_post_hoc_RR$report[45]`). Nevertheless, the SE group showed a lower RR compared to the NLE group (RR = `r RR_per_dx$mean_RR[6]`; SD = `r RR_per_dx$sd_RR[6]`; `r report_post_hoc_RR$report[39]`). As expected, the latter showed a higher RR than the acute seizures (RR = `r RR_per_dx$mean_RR[7]`; SD = `r RR_per_dx$sd_RR[7]`; `r report_post_hoc_RR$report[46]`), vaso-vagal syncopes (RR = `r RR_per_dx$mean_RR[9]`; SD = `r RR_per_dx$sd_RR[9]`; `r report_post_hoc_RR$report[48]`), cardiac (RR = `r RR_per_dx$mean_RR[2]`; SD = `r RR_per_dx$sd_RR[2]`; `r report_post_hoc_RR$report[49]`), cardiovascular (RR = `r RR_per_dx$mean_RR[11]`; SD = `r RR_per_dx$sd_RR[11]`; `r report_post_hoc_RR$report[50]`), and PNES groups (RR = `r RR_per_dx$mean_RR[7]`; SD = `r RR_per_dx$sd_RR[7]`; `r report_post_hoc_RR$report[51]`). Finally, patients who presented acute symptomatic seizures had higher RR than vaso-vagal syncopes (RR = `r RR_per_dx$mean_RR[9]`; SD = `r RR_per_dx$sd_RR[9]`; `r report_post_hoc_RR$report[53]`), cardiovascular (RR = `r RR_per_dx$mean_RR[11]`; SD = `r RR_per_dx$sd_RR[11]`; `r report_post_hoc_RR$report[55]`), and PNES patients (RR = `r RR_per_dx$mean_RR[12]`; SD = `r RR_per_dx$sd_RR[12]`; `r report_post_hoc_RR$report[56]`). 
Interestingly, patients for whom the index of the event was not the first one also significantly tend to have a higher RR (first event: RR = `r N_event_data_RR$mean_RR[N_event_data_RR$more_1_event == "one"]`; SD = `r N_event_data_RR$sd_RR[N_event_data_RR$more_1_event == "one"]`; not the first event: RR = `r N_event_data_RR$mean_RR[N_event_data_RR$more_1_event == "many"]`; SD = `r N_event_data_RR$sd_RR[N_event_data_RR$more_1_event == "many"]`; `r res_model_RR$report[12]`), stressing again the importance of a swift intervention after the first event.
...
plot.RR <- data_lm_RR %>%
  ...
  summarise(MEAN = round(mean(n.tot.recidives, na.rm = T),2),
            SD = round(sd(n.tot.recidives, na.rm = T),2))%>%
  ...
  geom_errorbar(aes(x = good_dx, ymin = MEAN-SD, ymax = MEAN+SD), col = "gray47", width = 0.5)+
  geom_text(aes(label = paste(round(MEAN,2), "(", round(SD,2),')', sep = '')), vjust = -1)+
  theme_minimal()+
...
conf_mat_MD <- table(data_MD$res.dx.initial, data_MD$res.dx.final)
acc_MD <- round(mean(data_MD$res.dx.initial == data_MD$res.dx.final),4)*100
